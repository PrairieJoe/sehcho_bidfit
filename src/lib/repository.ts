...
export class PublicRepository {
  ...
  async listNotices() { const topic = await this.getTopic(); const runStart = await latestCompletedRunStart(this.admin); let query = this.admin.from("notices").select("*, attachments(*, attachment_texts(*)), topic_scores!left(analysis, topic_id)").order("closes_at"); if (runStart) query = query.gte("updated_at", runStart); const { data, error } = await query; if (error) throw error; return (data ?? []).map((row: Row) => noticeOf({ ...row, topic_scores: (row.topic_scores as Row[] | undefined)?.filter((score) => score.topic_id === topic.id) })); }
