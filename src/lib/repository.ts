...
const noticeOf = (row: Row, state?: Row): BidNotice => {
  ...
  // Never show a score from an earlier run while the current attachment set is pending.
  const analysis = candidate && (!attachments.length || attachments.every((item) => item.status === "분석 완료")) ? candidate : undefined;
