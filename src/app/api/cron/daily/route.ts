...
export async function GET(request: NextRequest) {
  ...
  try { await ensureDefaultTopic(); return NextResponse.json(await runBatch()); } catch { return NextResponse.json({ message: "배치 실행에 실패했습니다. 운영 화면에서 이력을 확인하세요." }, { status: 500 }); }
}
