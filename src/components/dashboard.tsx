...
function Overview({ notices, eligible, topic, onRun, running, onOpen, onShowAll, isAdmin }: { notices: BidNotice[]; eligible: BidNotice[]; topic: Topic; onRun: () => void; running: boolean; onOpen: (notice: BidNotice) => void; onShowAll: () => void; isAdmin: boolean }) {
  ...
  return <div className="page-content">
    <section className="title-row"><div><p className="eyebrow">매일 08:00~09:00 KST 정기 수집</p><h1>오늘의 입찰 기회</h1><p className="lede"><strong>{topic.name}</strong> 주제와 관련된 공고를 분석했습니다. 원문 공고와 첨부문서를 최종 확인하세요.</p></div>{isAdmin && <button className="primary-button" onClick={onRun} disabled={running}><RefreshCw size={17} className={running ? "spin" : ""} />{running ? "분석 중…" : "오늘 분석 실행"}</button>}</section>
