...
function NoticeList({ notices, query, typeFilter, onQuery, onType, onOpen }: { notices: BidNotice[]; query: string; typeFilter: string; onQuery: (value: string) => void; onType: (value: string) => void; onOpen: (notice: BidNotice) => void }) {
  return <div className="page-content"><section className="title-row compact"><div><p className="eyebrow">검색 및 우선순위 확인</p><h1>전체 공고</h1></div></section><div className="filters"><label className="search"><Search size={18} /><input value={query} onChange={(event) => onQuery(event.target.value)} placeholder="공고명, 기관명 검색" /></label><label className="select"><Filter size={17} /><select value={typeFilter} onChange={(event) => onType(event.target.value)}><option>전체</option><option>용역</option><option>물품</option><option>공사</option><option>외자</option></select></label></div><div className="table-wrap"><table><thead><tr><th>적합도</th><th>공고</th><th>기관</th><th>예산</th><th>마감</th><th>상태</th><th /></tr></thead><tbody>{notices.map((notice) => <tr key={notice.id}><td><span className={`score-chip small ${scoreTone(notice.analysis?.score ?? 0)}`}>{notice.analysis ? notice.analysis.score : "미분석"}</span></td><td><button className="title-link" onClick={() => onOpen(notice)}>{notice.title}<small>{notice.bidNumber}-{notice.order} · {notice.businessType}</small></button></td><td>{notice.demandAgency}</td><td>{money(notice.budget)}</td><td><strong className={daysLeft(notice.closesAt) < 3 ? "urgent" : ""}>{notice.status === "마감" ? "마감" : notice.closesAt ? `D-${daysLeft(notice.closesAt)}` : "확인 필요"}</strong><small>{time(notice.closesAt)}</small></td><td><span className={`status-tag ${notice.status}`}>{notice.status}</span></td><td><div className="table-actions">{notice.detailUrl && notice.detailUrl !== "https://www.g2b.go.kr" && <a className="icon-button table-button" href={notice.detailUrl} target="_blank" rel="noreferrer" aria-label="나라장터 원문 열기">원문</a>}<button className="icon-button table-button" onClick={() => onOpen(notice)} aria-label="상세 보기"><ChevronRight size={18} /></button></div></td></tr>)}</tbody></table>{notices.length === 0 && <div className="empty-state">조건에 맞는 공고가 없습니다.</div>}</div></div>;
}
...
function Operations({ runs, notices }: { runs: BatchRun[]; notices: BidNotice[] }) { const success = notices.flatMap((item) => item.attachments).filter((item) => item.status === "분석 완료").length; const incomplete = notices.flatMap((item) => item.attachments).filter((item) => item.status !== "분석 완료").length; return <div className="page-content"><section className="title-row compact"><div><p className="eyebrow">Vercel Cron · 나라장터 API</p><h1>운영 현황</h1><p className="lede">매일 한 번, 수집부터 첨부문서 TXT 변환과 Gemini 분석까지 자동으로 실행합니다.</p></div></section><div className="operation-grid"><article><span>첨부 원문 분석 완료</span><strong>{success}</strong><small>TXT 변환과 Gemini 분석 근거가 확보된 문서</small></article><article><span>분석 미완료·보류 문서</span><strong>{incomplete}</strong><small>지원하지 않는 형식, 10MB 초과, 다운로드·텍스트 추출 실패 포함</small></article><article><span>데이터 공급자</span><strong>나라장터</strong><small>최근 72시간 중첩 수집</small></article></div><section className="section-head"><div><h2>최근 실행</h2><p>정기 Cron이 저장한 실행 이력입니다.</p></div></section><div className="table-wrap"><table><thead><tr><th>실행 시각</th><th>상태</th><th>발견</th><th>변경</th><th>분석</th><th>알림</th><th>API 호출</th></tr></thead><tbody>{runs.length ? runs.map((run) => <tr key={run.id}><td>{time(run.startedAt)}</td><td><span className="status-tag 신규">{run.status}</span></td><td>{run.discovered}</td><td>{run.changed}</td><td>{run.analyzed}</td><td>{run.notified}</td><td>{run.apiCalls}</td></tr>) : <tr><td colSpan={7} className="muted-cell">아직 자동 실행 이력이 없습니다.</td></tr>}</tbody></table></div></div>; }
