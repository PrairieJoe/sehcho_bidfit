...
function Operations({ runs, notices }: { runs: BatchRun[]; notices: BidNotice[] }) { const success = notices.flatMap((item) => item.attachments).filter((item) => item.status === "분석 완료").length; const partial = notices.flatMap((item) => item.attachments).filter((item) => item.status !== "분석 완료").length; return <div className="page-content"><section className="title-row compact"><div><p className="eyebrow">Vercel Cron · 나라장터 API</p><h1>운영 현황</h1><p className="lede">매일 자동 실행된 수집·키워드 점수화 결과입니다. 첨부 원문 분석은 아직 자동 실행 대상이 아닙니다.</p></div></section><div className="operation-grid"><article><span>첨부 원문 분석 완료</span><strong>{success}</strong><small>현재는 별도 기능 준비 중</small></article><article><span>미처리 첨부문서</span><strong>{partial}</strong><small>점수는 공고 기본정보 기준</small></article><article><span>데이터 공급자</span><strong>나라장터</strong><small>최근 72시간 중첩 수집</small></article></div><section className="section-head"><div><h2>최근 실행</h2><p>정기 Cron이 저장한 실행 이력입니다.</p></div></section><div className="table-wrap"><table><thead><tr><th>실행 시각</th><th>상태</th><th>발견</th><th>변경</th><th>분석</th><th>알림</th><th>API 호출</th></tr></thead><tbody>{runs.length ? runs.map((run) => <tr key={run.id}><td>{time(run.startedAt)}</td><td><span className="status-tag 신규">{run.status}</span></td><td>{run.discovered}</td><td>{run.changed}</td><td>{run.analyzed}</td><td>{run.notified}</td><td>{run.apiCalls}</td></tr>) : <tr><td colSpan={7} className="muted-cell">아직 자동 실행 이력이 없습니다.</td></tr>}</tbody></table></div></div>; }
