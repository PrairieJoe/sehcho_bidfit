...
나라장터 입찰공고를 공개 열람하고, 관리자가 지정한 주제와 규칙 기반 적합도를 확인하는 Vercel 공개 베타입니다.
...
- 현재 알림·이메일·매직링크·AI 분석은 제공하지 않습니다.
...
1. Supabase SQL Editor에서 `supabase/migrations/202608250001_initial_schema.sql`, `202608250002_admin_only_writes.sql`, `202608250003_public_read_admin_settings.sql`을 순서대로 실행합니다.
2. `bid-documents` 비공개 Storage 버킷이 생성되었는지 확인합니다.
...
배치는 최근 72시간 공고를 물품·용역·공사·외자 유형으로 조회하고, `입찰공고번호 + 공고차수`로 중복을 제거합니다. 첨부파일은 PDF/HWPX 중심으로 처리하며 지원하지 않는 파일은 실패 또는 보류 상태로 기록합니다.
